...
const Messages = ({ socket }) => {
    ...
    const handleSendMessage = async (e) => {
        ...
        const newMessage = {
            ...
            senderImage: user?.image || "default-image.jpg", // Ensure sender image exists
        ...
        try {
            const response = await fetch("http://localhost:2002/messages", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
                                    ...
                                    <div className='d-flex flex-column align-items-center'>
                                        {msg.senderId !== user._id && (
                                            <img src={msg.senderImage} alt={msg.senderUsername}
                                                 className="rounded-circle mr-2 me-2" style={{ width: 30, height: 30, objectFit: "cover" }} />
                                        )}
                                        <div className="d-flex me-2">
                                            {(msg.senderId === user._id || user.username === "beyka") && (
                                                <button
                                                    className="btn btn-sm"
                                                    onClick={() => deleteMessages(msg._id)} // Pass the correct messageId
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" height="24px"
                                                         viewBox="0 -960 960 960" width="24px" fill="#8B1A10">
                                                        <path
